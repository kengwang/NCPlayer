    ...
    public class SyllablesRenderingLyricLine : RenderingLyricLine
    {
        ...
        public override bool Render(CanvasDrawingSession session, LineRenderOffset offset, RenderContext context)
        {
                ...
                using (var clds = cl.CreateDrawingSession())
                {
                    ...
                    if (tll != null)
                    {
                        if (_isFocusing && context.Effects.TransliterationScanning)
                        {
                            ...
                            {
                                clds.DrawTextLayout(tll, (float)(textLayout.LayoutBounds.X + actualX), actualTop, idleColor);
                                ...
                                if (highlightGeometry.geo2 is not null) //填充渐变矩形
                                {
                                    ...
                                    var highlightTextGeometry2 = highlightGeometry.geo2.CombineWith(textGeometry,
                                        Matrix3x2.Identity,
                                        CanvasGeometryCombine.Intersect);
                                    clds.FillGeometry(highlightTextGeometry2, (float)(textLayout.LayoutBounds.X + actualX), actualTop, color);
                                }

                                clds.FillGeometry(highlightTextGeometry, (float)(textLayout.LayoutBounds.X + actualX), actualTop,
                                    TypographySelector(t => t?.FocusingColor, context)!.Value);
                            }
                            else
                            {
                                clds.DrawTextLayout(tll, (float)(textLayout.LayoutBounds.X + actualX), actualTop,
                                    TypographySelector(t => t?.FocusingColor, context)!.Value);
                            }
                        }
                        else
                    ...
                    if (tl != null)
                    {
                        clds.DrawTextLayout(tl, (float)(textLayout.LayoutBounds.X + actualX), actualTop,
                            _isFocusing
                                ? TypographySelector(t => t?.FocusingColor, context)!.Value
                                : idleColor);
                    }
            ...
            if (_reactionState == ReactionState.Enter)
            {
                ...
                session.FillRoundedRectangle(actualX - (float)textLayout.LayoutBounds.X, offset.Y,
                    RenderingWidth + actualX, RenderingHeight, 6, 6, color);
            }
            ...
            if (context.Debug)
            {
                ...
                session.DrawRectangle((float)textLayout.LayoutBounds.X + actualX, drawingTop, RenderingWidth, RenderingHeight, Colors.Yellow);
            }
                ...
                _isRomajiSyllable)
            {
                ...
                if (textLayout is null || _sizeChanged)
                {
                    ...
                    textLayout = new CanvasTextLayout(session, _text, textFormat,
                        Math.Clamp(_canvasWidth - 12, 0, int.MaxValue), _canvasHeight);
                ...
                requestWidth ??=  (float?)textLayout?.LayoutBounds.Width; //有歌词的时候优先用歌词的宽度
                if (!string.IsNullOrWhiteSpace(Translation))
                {
                    ...
                    tl = new CanvasTextLayout(session, Translation, translationFormat, requestWidth ?? _canvasWidth - 12, _canvasHeight);
                    add += 30;
                }
                requestWidth ??= (float?)tl?.LayoutBounds.Width; //没有歌词但有翻译的时候优先用翻译的宽度
                if (!string.IsNullOrWhiteSpace(Transliteration))
                {
                    ...
                    if (!_isInitialized)
                        _transliterationActual = _isRomajiSyllable
                            ...
                            : Transliteration;
                    tll = new CanvasTextLayout(session, _transliterationActual, transliterationFormat, requestWidth ?? _canvasWidth - 12,_canvasHeight); //啥都没有，你这个词给的很离谱啊
                    add += 10;
                }
